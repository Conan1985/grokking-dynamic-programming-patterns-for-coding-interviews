...
class EditDistanceBruteForce {
    ...
    private int findMinOperationsRecursive(String s1, String s2, int i1, int i2) {
        ...
        int c1 = 1 + findMinOperationsRecursive(s1, s2, i1 + 1, i2 + 1);
        int c2 = 1 + findMinOperationsRecursive(s1, s2, i1 + 1, i2);
        int c3 = 1 + findMinOperationsRecursive(s1, s2, i1, i2 + 1);
        return Math.min(c1, Math.min(c2, c3));
    }

    public static void main(String[] args) {
        EditDistanceBruteForce editDisatnce = new EditDistanceBruteForce();
